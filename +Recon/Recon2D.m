...
% columns for x,y, and z (3 x N_samples x N_excitations). The z-dimension
% must be set to zeros for the 2D reconstruction.
%
...
% vice versa.
%
% PixelShift - Array of size 2 to shift pixels by ([shift_x, shift_y]).
%
%
% Outputs:
%
...
% figure; [Image_Out, DCF] = Recon.Recon2D(116,data,traj,[],[],116,[]); imagesc(abs(Image_Out(:,:,1)))
...
Image = Image(:,:,1); % In 2D reconstruction, we only need 1 slice (all slices are identical). 
Image = circshift(circshift(Image,round(PixelShift(1)),1),round(PixelShift(2)),2);

%% Crop:
xs = floor(ReconSize - ReconSize/alpha)+1;
xe = floor(ReconSize + ReconSize/alpha);
Image_Out = Image(xs:xe,xs:xe,:);
